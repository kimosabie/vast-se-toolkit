...
def render():
    ...
    def _render_net_diagram_svg(
        ...
        Layered top-to-bottom (no left labels):
        ...
        HGAP         = 22
        VGAP         = 90
        MARGIN       = 24         # left/right canvas margin (no label column)
        W            = 1200
        CX           = W // 2
        AVAIL        = W - 2 * MARGIN
        ...
        def _sep(y):
            return _line(MARGIN, y, W - MARGIN, y,
                         C["sep"], lw=1, dash="10,7")
        ...
        if custom_devices:
            # Use devices from Device Inventory (non-VAST customer hardware)
            ...
            for cx, dev in zip(srv_ctrs, shown_devs):
                ...
                if dev_b64:
                    shapes.append(_device_box(cx, y, nm, sub,
                                               C["server_fill"], dev_b64))
                else:
            ...
            if extra_srv > 0:
                ...
                bw, bh = 56, DH
                shapes.append(_rect(bx, y, bw, bh, "#dde2f0",
                                     stroke="#aab4cc", rx=6))
                shapes.append(_txt(bx + bw // 2, y + bh // 2,
                                    f"+{extra_srv}", size=12,
                                    fill=C["text_dark"], bold=True))
        else:
            # Generic fallback — two generic server boxes
            srv_ctrs = _row_cx(2)
        ...
        if gpu_enabled:
            ...
            dsw_svg, dsw_ctrs = _device_row(
                dsw_names, dsw_ms, 2, _gpu_img, C["data_fill"], y
            )
        ...
        cn_svg, cn_ctrs = _device_row(
            cn_names, cn_ms, num_cnodes, _cnode_img, C["cnode_fill"], y
        )
        ...
        if topology == "Spine-Leaf":
            ...
            sp_svg, sp_ctrs = _device_row(
                sp_names, sp_ms, 2, _spine_img, C["spine_fill"], y
            )
        ...
        lf_svg, lf_ctrs = _device_row(
            lf_names, lf_ms, 2, _sw_img, C["leaf_fill"], y
        )
        shapes.append(lf_svg)
        # ISL between leaf switches
        isl_y  = y + DH // 2
        isl_x1 = int(lf_ctrs[0] + DW // 2)
        isl_x2 = int(lf_ctrs[1] - DW // 2)
        shapes.append(_line(isl_x1, isl_y, isl_x2, isl_y,
                             C["isl"], lw=2.5, dash="8,4"))
        shapes.append(_txt((isl_x1 + isl_x2) // 2, isl_y - 8,
                            "ISL / Peerlink", size=9,
                            fill=C["isl"], bold=True))
        ...
        db_svg, db_ctrs = _device_row(
            db_names, db_ms, num_dboxes, _dbox_img, C["dbox_fill"], y
        )
        ...
        shapes.append(_rect(LX, LY, 242, leg_h,
                             C["cloud_fill"], stroke=C["cloud_stroke"], rx=6))
        shapes.append(_txt(LX + 121, LY + 12, "Legend",
                            size=10, fill=C["text_dark"], bold=True))
        for li, (clr, lw, dsh, lbl) in enumerate(leg_items):
            ...
            shapes.append(_txt(LX + 44, ly2 + 1, lbl,
                                size=10, fill=C["text_dark"], anchor="start"))
    ...
    _nd_dbox_lbl    = st.session_state.get("name_dbox",      "DBOX")
    _nd_cnode_lbl   = st.session_state.get("name_cnode",     "CNODE")
    _nd_custom_devs = st.session_state.get("rack_custom_devices", [])

    _nd_svg, _nd_w, _nd_h = _render_net_diagram_svg(
        pfx           = pfx,
        ...
        dbox_label    = _nd_dbox_lbl,
        cnode_label   = _nd_cnode_lbl,
        custom_devices= _nd_custom_devs,
    )

    # Display — embed as base64 SVG in an <img> tag via st.markdown.
    # This scales to 100% container width with height:auto — no iframe height
    # maths, no cairosvg required for display, no scrolling.
