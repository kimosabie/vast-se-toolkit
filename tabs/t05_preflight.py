...
def render():
    ...
    # ── Pull switch state (shared with t04_confluence) ───────────
    sw              = get_switch_ctx(pfx)
    profile_t3      = sw["profile"]
    sw_model_t3     = sw["sw_model"]
    vlan_t3         = sw["vlan"]
    sw_a_t3         = sw["sw_a_ip"]
    sw_b_t3         = sw["sw_b_ip"]
    ntp_t3          = sw["ntp"]
    isl_t3          = sw["isl"]
    dnode_st_t3     = sw["dnode_start"]
    cnode_st_t3     = sw["cnode_start"]
    isl_short_t3    = sw["isl_short"]
    isl_list_t3     = sw["isl_list"]
    isl_cable_t3    = sw["isl_cable"]
    gpu_enabled_t3  = sw["gpu_enabled"]
    sw_a_ip_only    = sw["sw_a_ip_only"]
    sw_b_ip_only    = sw["sw_b_ip_only"]
    proc_gpu_sw_a   = sw["gpu_sw_a_ip"]
    proc_gpu_sw_b   = sw["gpu_sw_b_ip"]
    proc_gpu_model  = sw["gpu_model"]
    proc_gpu_profile = sw["gpu_profile"]
    today_str       = sw["today_str"]
    fname_sw_a      = sw["fname_sw_a"]
    fname_sw_b      = sw["fname_sw_b"]
    fname_gpu_sw_a  = sw["fname_gpu_a"]
    fname_gpu_sw_b  = sw["fname_gpu_b"]
    spine_enabled   = (topology == "Spine-Leaf")
    proc_spine_model   = sw["spine_model"]
    proc_spine_profile = sw["spine_profile"]
    fname_spine_a   = sw["fname_sp_a"]
    fname_spine_b   = sw["fname_sp_b"]
    proc_spine_a_ip = sw["spine_a_ip"]
    proc_spine_b_ip = sw["spine_b_ip"]
...
Switch:   {sw_model_t3}
Topology: {topology}
Generated by: VAST SE Toolkit v{APP_VERSION}
